<template>
  <div>
    <div  class="center" v-for="(item, index) in rawData" >
      <svg :id="item.indexname" width="30%" height="200" ></svg> {{rawData[index].name}}
    </div>
</div>
</template>
<script>
        ...
        updated() {
          for (let index in this.rawData)
          {
              ...
              const options = {
                'page[number]': 1,
                'page[size]': 1,
                'filter[sensor_id]': this.rawData[index].sensor_id,
                'filter[key]': this.rawData[index].sensor_key,
                'sort' : "-created_at"
              }
      ...
      methods: {
        ...
        prepare(){
            ...
            for(let key_index in sensor_data_key)
            {
                ...
                prepare_chartdata.indexname="index"+sensor_key;
                prepare_chartdata.sensor_id=sensor_id;
                prepare_chartdata.sensor_key=sensor_key;
                prepare_chartdata.color=sensor_data_key[key_index][sensor_key].color.hex;
        ...
        getSensorData(sensor_id,sensor_key){
          const options = {
            ...
            this.$store.dispatch('sensorDatas/loadPage', { options }).then(() => {
                const sensorPageRaw = this.$store.getters['sensorDatas/page'];
                this.rawData.sensor=sensorPageRaw;
